class SelfTrainingClassifier(ClassifierMixin, MetaEstimatorMixin, BaseEstimator):
    ...
    """
    _parameter_constraints: dict = {'estimator': [None, HasMethods(['fit'])], 'base_estimator': [HasMethods(['fit']), Hidden(StrOptions({'deprecated'}))], 'threshold': [Interval(Real, 0.0, 1.0, closed='left')], 'criterion': [StrOptions({'threshold', 'k_best'})], 'k_best': [Interval(Integral, 1, None, closed='left')], 'max_iter': [Interval(Integral, 0, None, closed='left'), None], 'verbose': ['verbose']}

    def __init__(self, estimator=None, base_estimator='deprecated', threshold=0.75, criterion='threshold', k_best=10, max_iter=10, verbose=False):
    ...
    def fit(self, X, y, **params):
        ...
        **params : dict
        ...
        while not np.all(has_label) and (self.max_iter is None or self.n_iter_ < self.max_iter):
            self.n_iter_ += 1
            self.estimator_.fit(X[safe_mask(X, has_label)], self.transduction_[has_label], **routed_params.estimator.fit)
